...
extension AgoraLectureRenderUIController {
    func updateRenderView(isFullScreen: Bool) {
        // 全屏的时候， 从1变成0
        teacherView.alpha = isFullScreen ? 1 : 0
    ...
    func updateRenderView(hasCoHosts: Bool) {
        renderListView.isHidden = !hasCoHosts
        renderListView.alpha = hasCoHosts ? 0 : 1
        
        UIView.animate(withDuration: TimeInterval.agora_animation) {
            self.renderListView.alpha = hasCoHosts ? 1 : 0
        }
    }
}
...
extension AgoraLectureRenderUIController: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView,
        ...
        userView.updateMicState(userInfo.microState.uiType,
        ...
        userView.updateUserName(name: userInfo.user.userName)
