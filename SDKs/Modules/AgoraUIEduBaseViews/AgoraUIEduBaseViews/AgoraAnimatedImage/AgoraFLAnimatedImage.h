...
extern const NSTimeInterval kAgoraFLAnimatedImageDelayTimeIntervalMinimum;

//
//  An `AgoraFLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `FLAnimatedImageView`.
...
//
@interface AgoraFLAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `AgoraFLAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `AgoraFLAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
typedef NS_ENUM(NSUInteger, AgoraFLLogLevel) {
    AgoraFLLogLevelNone = 0,
    AgoraFLLogLevelError,
    AgoraFLLogLevelWarn,
    AgoraFLLogLevelInfo,
    AgoraFLLogLevelDebug,
    AgoraFLLogLevelVerbose
};

@interface AgoraFLAnimatedImage (Logging)
...
#define AgoraFLLog(logLevel, format, ...) [AgoraFLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface AgoraFLWeakProxy : NSProxy
