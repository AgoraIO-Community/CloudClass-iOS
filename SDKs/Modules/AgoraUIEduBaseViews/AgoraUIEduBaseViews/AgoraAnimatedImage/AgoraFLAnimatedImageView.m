...
#import "AgoraFLAnimatedImageView.h"
#import "AgoraFLAnimatedImage.h"
#import <QuartzCore/QuartzCore.h>


#if defined(DEBUG) && DEBUG
@protocol AgoraFLAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(AgoraFLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface AgoraFLAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<AgoraFLAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation AgoraFLAnimatedImageView
...
- (void)setAnimatedImage:(AgoraFLAnimatedImage *)animatedImage
{
    if (![_animatedImage isEqual:animatedImage]) {
        if (animatedImage) {
            if (super.image) {
                ...
                // See more here: https://github.com/Flipboard/AgoraFLAnimatedImage/issues/100
...
{
    // Presision is set to half of the `kAgoraFLAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kAgoraFLAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            AgoraFLWeakProxy *weakProxy = [AgoraFLWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    // Early return!
    if (!self.shouldAnimate) {
        AgoraFLLog(AgoraFLLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber != nil) {
        ...
        if (image) {
            AgoraFLLog(AgoraFLLogLevelVerbose, @"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
            ...
            AgoraFLLog(AgoraFLLogLevelDebug, @"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
