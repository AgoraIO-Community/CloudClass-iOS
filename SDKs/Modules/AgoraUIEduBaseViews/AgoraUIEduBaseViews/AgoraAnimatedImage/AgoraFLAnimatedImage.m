...
const NSTimeInterval kAgoraFLAnimatedImageDelayTimeIntervalMinimum = 0.02;
...
typedef NS_ENUM(NSUInteger, AgoraFLAnimatedImageDataSizeCategory) {
    AgoraFLAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    AgoraFLAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    AgoraFLAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    AgoraFLAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, AgoraFLAnimatedImageFrameCacheSize) {
    AgoraFLAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    AgoraFLAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    AgoraFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    AgoraFLAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol AgoraFLAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(AgoraFLAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(AgoraFLAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(AgoraFLAnimatedImage *)animatedImage;
@end
#endif


@interface AgoraFLAnimatedImage ()
...
// The actual type of the object is `AgoraFLWeakProxy`.
@property (nonatomic, strong, readonly) AgoraFLAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<AgoraFLAnimatedImageDebugDelegate> debug_delegate;
#endif
...
@implementation AgoraFLAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > AgoraFLAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > AgoraFLAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [AgoraFLAnimatedImage class]) {
...
- (instancetype)init
{
    AgoraFLAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        AgoraFLLog(AgoraFLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `AgoraFLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (!hasData) {
        AgoraFLLog(AgoraFLLogLevelError, @"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            AgoraFLLog(AgoraFLLogLevelError, @"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            AgoraFLLog(AgoraFLLogLevelError, @"Supplied data is of type %@ and doesn't seem to be GIF data %@", imageSourceContainerType, data);
            return nil;
        }
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if (delayTime == nil) {
                            if (i == 0) {
                                AgoraFLLog(AgoraFLLogLevelInfo, @"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                                delayTime = @(kDelayTimeIntervalDefault);
                            } else {
                                AgoraFLLog(AgoraFLLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                        ...
                        if ([delayTime floatValue] < ((float)kAgoraFLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            AgoraFLLog(AgoraFLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kAgoraFLAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
                        delayTimesForIndexesMutable[@(i)] = delayTime;
                    } else {
                        skippedFrameCount++;
                        AgoraFLLog(AgoraFLLogLevelInfo, @"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
                    }
                    CFRelease(frameImageRef);
                } else {
                    skippedFrameCount++;
                    AgoraFLLog(AgoraFLLogLevelInfo, @"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, _imageSource);
                }
            }
        }
        ...
        if (self.frameCount == 0) {
            AgoraFLLog(AgoraFLLogLevelInfo, @"Failed to create any valid frames for GIF with properties %@", imageProperties);
            return nil;
        } else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            AgoraFLLog(AgoraFLLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        } else {
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= AgoraFLAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= AgoraFLAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = AgoraFLAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = AgoraFLAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
        ...
        _weakProxy = (id)[AgoraFLWeakProxy weakProxyForObject:self];
...
{
    AgoraFLAnimatedImage *animatedImage = [[AgoraFLAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index
{
    ...
    if (index >= self.frameCount) {
        AgoraFLLog(AgoraFLLogLevelWarn, @"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
        return nil;
    }
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    if (firstRange.length + secondRange.length != self.frameCount) {
        AgoraFLLog(AgoraFLLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    }
    ...
    if (!self.serialQueue) {
        _serialQueue = dispatch_queue_create("com.flipboard.framecachingagoraqueue", DISPATCH_QUEUE_SERIAL);
    }
    ...
    AgoraFLAnimatedImage * __weak weakSelf = self;
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                AgoraFLLog(AgoraFLLogLevelVerbose, @"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i, (predrawDuration + slowdownDuration) * 1000, self);
#endif
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[AgoraFLAnimatedImage class]]) {
        AgoraFLAnimatedImage *animatedImage = (AgoraFLAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        AgoraFLLog(AgoraFLLogLevelError, @"`image` isn't of expected types `UIImage` or `AgoraFLAnimatedImage`: %@", image);
    }
...
- (NSMutableIndexSet *)frameIndexesToCache
{
    ...
    if (self.frameCacheSizeCurrent == self.frameCount) {
        ...
        if ([indexesToCache count] != self.frameCacheSizeCurrent) {
            AgoraFLLog(AgoraFLLogLevelWarn, @"Number of frames to cache doesn't equal expected cache size.");
        }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize
{
    ...
    AgoraFLLog(AgoraFLLogLevelDebug, @"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
...
{
    self.frameCacheSizeMaxInternal = AgoraFLAnimatedImageFrameCacheSizeNoLimit;
    AgoraFLLog(AgoraFLLogLevelDebug, @"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(AgoraFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    AgoraFLLog(AgoraFLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)AgoraFLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = AgoraFLAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(AgoraFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw
{
    ...
    if (!colorSpaceDeviceRGBRef) {
        AgoraFLLog(AgoraFLLogLevelError, @"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
    ...
    // Early return on failure!
    if (!bitmapContextRef) {
        AgoraFLLog(AgoraFLLogLevelError, @"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: %zu height: %zu bitsPerComponent: %zu bytesPerRow: %zu) for image %@", colorSpaceDeviceRGBRef, width, height, bitsPerComponent, bytesPerRow, imageToPredraw);
        return imageToPredraw;
    }
    ...
    // Early return on failure!
    if (!predrawnImage) {
        AgoraFLLog(AgoraFLLogLevelError, @"Failed to `imageWithCGImage:scale:orientation:` with image ref %@ created with color space %@ and bitmap context %@ and properties and properties (scale: %f orientation: %ld) for image %@", predrawnImageRef, colorSpaceDeviceRGBRef, bitmapContextRef, imageToPredraw.scale, (long)imageToPredraw.imageOrientation, imageToPredraw);
        return imageToPredraw;
    }
...
@implementation AgoraFLAnimatedImage (Logging)

static void (^_logBlock)(NSString *logString, AgoraFLLogLevel logLevel) = nil;
static AgoraFLLogLevel _logLevel;

+ (void)setLogBlock:(void (^)(NSString *logString, AgoraFLLogLevel logLevel))logBlock logLevel:(AgoraFLLogLevel)logLevel
{
    _logBlock = logBlock;
    _logLevel = logLevel;
}

+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(AgoraFLLogLevel)level
{
...
#pragma mark - AgoraFLWeakProxy

@interface AgoraFLWeakProxy ()
...
@implementation AgoraFLWeakProxy

#pragma mark Life Cycle

// This is the designated creation method of an `AgoraFLWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject
{
    AgoraFLWeakProxy *weakProxy = [AgoraFLWeakProxy alloc];
