...
@interface AgoraRTCManager()<AgoraRtcEngineDelegate, AgoraRtcChannelDelegate>
...
- (void)initEngineKitWithAppid:(NSString *)appid {
    ...
    [AgoraRTELogService logMessageWithDescribe:@"init rtcEngineKit appid:" message:NoNullString(appid)];
...
- (int)joinChannelByToken:(NSString * _Nullable)token channelId:(NSString * _Nonnull)channelId info:(NSString * _Nullable)info uid:(NSUInteger)uid autoSubscribeAudio:(BOOL)autoSubscribeAudio autoSubscribeVideo:(BOOL)autoSubscribeVideo {
    ...
    [AgoraRTELogService logMessageWithDescribe:@"join channel:" message:@{@"roomUuid":NoNullString(channelId), @"token":NoNullString(token), @"uid":@(uid)}];
    ...
    for(RTCChannelInfo *channelInfo in self.rtcChannelInfos) {
        if (!(channelInfo.agoraRtcChannel && [channelInfo.channelId isEqualToString:channelId])) {
            if (channelInfo.isPublish) {
                ...
                [AgoraRTELogService logMessageWithDescribe:@"unpublish:" message:@{@"roomUuid":NoNullString(channelId), @"code":@(code)}];
                if (code == 0) {
                    channelInfo.isPublish = NO;
                } else {
                    return code;
        ...
        if(!currentChannelInfo.isPublish) {
            if(currentChannelInfo.role != AgoraClientRoleBroadcaster) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    [AgoraRTELogService logMessageWithDescribe:@"publish:" message:@{@"roomUuid":NoNullString(currentChannelInfo.channelId), @"code":@(code)}];
...
- (int)muteRemoteAudioStream:(NSString *)uid mute:(BOOL)mute channelId:(NSString *)channelId {
    ...
    for(RTCChannelInfo *channelInfo in self.rtcChannelInfos) {
        if (channelInfo.agoraRtcChannel && [channelInfo.channelId isEqualToString:channelId]) {
            ...
            [AgoraRTELogService logMessageWithDescribe:@"muteRemoteAudioStream:" message:@{@"roomUuid":NoNullString(channelId), @"uid":NoNullString(uid), @"mute":@(mute), @"code":@(code)}];
...
- (int)muteRemoteVideoStream:(NSString *)uid mute:(BOOL)mute channelId:(NSString *)channelId {
    ...
    for(RTCChannelInfo *channelInfo in self.rtcChannelInfos) {
        if (channelInfo.agoraRtcChannel && [channelInfo.channelId isEqualToString:channelId]) {
            ...
            [AgoraRTELogService logMessageWithDescribe:@"muteRemoteVideoStream:" message:@{@"roomUuid":NoNullString(channelId), @"uid":NoNullString(uid), @"mute":@(mute), @"code":@(code)}];
...
- (int)rate:(NSString *)callId rating:(NSInteger)rating description:(NSString *)description {
    ...
    [AgoraRTELogService logMessageWithDescribe:@"rate:" message:@{@"callId":NoNullString(callId), @"rating":@(rating), @"description":NoNullString(description), @"rate":@(rate)}];
...
- (int)startAudioMixing:(NSString *  _Nonnull)filePath loopback:(BOOL)loopback replace:(BOOL)replace cycle:(NSInteger)cycle {
    ...
    [AgoraRTELogService logMessageWithDescribe:@"startAudioMixing:" message: @{@"filePath":NoNullString(filePath), @"loopback":@(loopback), @"replace":@(replace), @"cycle":@(cycle), @"code":@(code)}];
...
- (int)setParameters:(NSString * _Nonnull)options {
    ...
    [AgoraRTELogService logMessageWithDescribe:@"setParameters:" message:@{@"options":NoNullString(options), @"code":@(code)}];
...
- (void)destoryWithChannelId:(NSString *)channelId {
    [AgoraRTELogService logMessageWithDescribe:@"desotry rtc:" message:@{@"roomUuid": NoNullString(channelId)}];
...
- (void)rtcChannelDidJoinChannel:(AgoraRtcChannel * _Nonnull)rtcChannel
    ...
    [AgoraRTELogService logMessageWithDescribe:@"didJoinChannel:" message:@{@"roomUuid":NoNullString(rtcChannel.getChannelId), @"uid":@(uid), @"elapsed":@(elapsed)}];
            ...
            if (channelInfo.agoraRtcChannel && [channelInfo.channelId isEqualToString:[rtcChannel getChannelId]]) {
                if (channelInfo.isPublish) {
                    ...
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        ...
                        [AgoraRTELogService logMessageWithDescribe:@"publish after role change end:" message:@{@"roomUuid":NoNullString([rtcChannel getChannelId]), @"code":@(code)}];
                    });
...
- (void)rtcChannel:(AgoraRtcChannel * _Nonnull)rtcChannel
    didJoinedOfUid:(NSUInteger)uid
           elapsed:(NSInteger)elapsed {
    [AgoraRTELogService logMessageWithDescribe:@"didJoinedOfUid:" message:@{@"roomUuid":NoNullString(rtcChannel.getChannelId), @"uid":@(uid), @"elapsed":@(elapsed)}];
...
- (void)rtcChannel:(AgoraRtcChannel * _Nonnull)rtcChannel
   didOfflineOfUid:(NSUInteger)uid
    ...
    [AgoraRTELogService logMessageWithDescribe:@"didOfflineOfUid:" message:@{@"roomUuid":NoNullString(rtcChannel.getChannelId), @"uid":@(uid), @"reason":@(reason)}];
