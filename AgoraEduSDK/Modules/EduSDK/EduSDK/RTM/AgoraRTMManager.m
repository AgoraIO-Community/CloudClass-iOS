...
- (void)initSignalWithAppid:(NSString *)appId appToken:(NSString *)appToken userId:(NSString *)uid completeSuccessBlock:(void (^ _Nullable) (void))successBlock completeFailBlock:(void (^ _Nullable) (NSInteger errorCode))failBlock {
    ...
    [self.agoraRtmKit loginByToken:appToken user:uid completion:^(AgoraRtmLoginErrorCode errorCode) {
        if (errorCode == AgoraRtmLoginErrorOk) {
            [AgoraRTELogService logMessageWithDescribe:@"rtm login success:" message:@{@"appId":NoNullString(appId), @"appToken":NoNullString(appToken), @"uid":NoNullString(uid)}];

            if (successBlock != nil) {
                successBlock();
            }
        } else {
            [AgoraRTELogService logErrMessageWithDescribe:@"rtm login failure:" message:@{@"errorCode":@(errorCode), @"appId":NoNullString(appId), @"appToken":NoNullString(appToken), @"uid":NoNullString(uid)}];
...
- (void)joinSignalWithChannelName:(NSString *)channelName completeSuccessBlock:(void (^ _Nullable) (void))successBlock completeFailBlock:(void (^ _Nullable) (NSInteger errorCode))failBlock {
    ...
    [AgoraRTELogService logMessageWithDescribe:@"join signal:" message:@{@"roomUuid":NoNullString(channelName)}];
    ...
    [agoraRtmChannel joinWithCompletion:^(AgoraRtmJoinChannelErrorCode errorCode) {
        ...
        if(errorCode == AgoraRtmJoinChannelErrorOk || errorCode == AgoraRtmJoinChannelErrorAlreadyJoined) {
            ...
            [AgoraRTELogService logErrMessageWithDescribe:@"rtm join channel success:" message:@{@"roomUuid":NoNullString(channelName)}];
            if(successBlock != nil) {
                successBlock();
            }
        } else {
            [AgoraRTELogService logErrMessageWithDescribe:@"rtm join channel failure:" message:@{@"roomUuid":NoNullString(channelName), @"errorCode":@(errorCode)}];
            if(failBlock != nil){
                failBlock(errorCode);
            }
        }
...
- (void)destoryWithChannelId:(NSString *)channelId {
    [AgoraRTELogService logMessageWithDescribe:@"desotry rtm:" message:@{@"roomUuid": NoNullString(channelId)}];
...
- (void)rtmKit:(AgoraRtmKit * _Nonnull)kit messageReceived:(AgoraRtmMessage * _Nonnull)message fromPeer:(NSString * _Nonnull)peerId {
    ...
    [AgoraRTELogService logMessageWithDescribe:@"rtm messageReceived:" message:@{@"message":NoNullString(message.text), @"peerId":NoNullString(peerId)}];
