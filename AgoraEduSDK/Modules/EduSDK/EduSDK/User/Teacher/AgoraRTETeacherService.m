...
- (void)allowStudentChat:(BOOL)enable remoteUser:(AgoraRTEUser *)remoteUser success:(AgoraRTESuccessBlock)successBlock failure:(AgoraRTEFailureBlock _Nullable)failureBlock {
    ...
    [AgoraRTEHttpManager updateUserStateWithRoomUuid:self.channelId userUuid:NoNullString(remoteUser.userUuid) userToken:self.userToken param:param apiVersion:APIVersion1 analysisClass:AgoraRTECommonModel.class success:^(id<AgoraRTEBaseModel>  _Nonnull objModel) {
        ...
        if(successBlock){
            ...
            if ([cm.data isKindOfClass:NSArray.class]) {
                for(id obj in cm.data) {
                    AgoraRTEStreamsChangedModel *model = [AgoraRTEStreamsChangedModel yy_modelWithDictionary:NoNullDictionary(obj)];
                    if (model != nil) {
