...
#import "UIView+AgoraEduBadge.h"

@interface AgoraMCSegmentedView ()
@property (nonatomic, copy) NSArray *items;
@end
@implementation AgoraMCSegmentedView

- (instancetype)initWithCoder:(NSCoder *)coder
{
    self = [super initWithCoder:coder];
    if (self) {
        self.items = [NSArray arrayWithObjects:AgoraEduLocalizedString(@"ChatroomText", nil), AgoraEduLocalizedString(@"StudentListText", nil), nil];
    }
    return self;
}
- (void)setSegmentType:(SegmentType)segmentType {
    if (segmentType == SegmentTypeList) {
        self.items = [NSArray arrayWithObjects:AgoraEduLocalizedString(@"StudentListText", nil), AgoraEduLocalizedString(@"ChatroomText", nil), nil];
