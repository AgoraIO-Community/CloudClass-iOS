...
@interface AgoraMCStudentVideoListView ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation AgoraMCStudentVideoListView
...
- (nonnull __kindof UICollectionViewCell *)collectionView:(nonnull UICollectionView *)collectionView cellForItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
    AgoraMCStudentVideoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"VideoCell" forIndexPath:indexPath];
    
    AgoraRTEStream *currentModel = self.studentArray[indexPath.row];
...
- (void)updateStudentArray:(NSArray<AgoraRTEStream*> *)studentArray {
    ...
    if(studentArray.count == 0 || self.studentArray.count != studentArray.count) {
        ...
        for(NSInteger i = 0; i < count; i++) {
            AgoraRTEStream *sourceModel = [self.studentArray objectAtIndex:i];
            AgoraRTEStream *currentModel = [studentArray objectAtIndex:i];
...
- (UICollectionView *)videoListView {
    if (!_videoListView) {
        ...
        [_videoListView registerClass:[AgoraMCStudentVideoCell class] forCellWithReuseIdentifier:@"VideoCell"];
    }
