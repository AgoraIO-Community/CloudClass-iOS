...
- (instancetype)init
{
    ...
    if (!animatedImage) {
        FLLog(FLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `AgoraFLAnimatedImage`.");
    }
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if ([delayTime floatValue] < ((float)kAgoraFLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(FLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kAgoraFLAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    if ([image isKindOfClass:[UIImage class]]) {
        ...
        FLLog(FLLogLevelError, @"`image` isn't of expected types `UIImage` or `AgoraFLAnimatedImage`: %@", image);
    }
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    FLLog(FLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)AgoraFLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
