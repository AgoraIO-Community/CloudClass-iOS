...
+ (void)getConfig:(AgoraRoomConfiguration *)config success:(OnConfigSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {
    ...
    NSDictionary *headers = [AgoraHTTPManager headersWithUId:config.userUuid userToken:nil token:config.token];
    
    [AgoraHTTPManager fetchDispatch:HttpTypeGet url:url parameters:@{} headers:headers parseClass:AgoraConfigModel.class success:^(id _Nonnull model) {
...
+ (void)roomStateWithConfig:(AgoraRoomStateConfiguration *)config  success:(OnRoomStateSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {
    ...
    [AgoraHTTPManager fetchDispatch:HttpTypePut url:url parameters:parameters headers:headers parseClass:AgoraRoomStateModel.class success:^(id _Nonnull model) {
        if(successBlock){
            successBlock(model);
        }
    } failure:^(NSError * _Nonnull error, NSInteger statusCode) {
...
+ (void)roomChatWithConfig:(AgoraRoomChatConfiguration *)config  success:(OnRoomChatSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {
    ...
    NSDictionary *headers = [AgoraHTTPManager headersWithUId:config.userUuid  userToken:config.userToken token:config.token];
    ...
    [AgoraHTTPManager fetchDispatch:HttpTypePost url:url parameters:parameters headers:headers parseClass:AgoraRoomStateModel.class success:^(id _Nonnull model) {
        if(successBlock){
            successBlock(model);
        }
    } failure:^(NSError * _Nonnull error, NSInteger statusCode) {
...
+ (void)handUpWithConfig:(AgoraHandUpConfiguration *)config  success:(OnHandUpSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {
    ...
    [AgoraHTTPManager fetchDispatch:HttpTypePost url:url parameters:parameters headers:headers parseClass:AgoraHandUpModel.class success:^(id _Nonnull model) {
        if(successBlock){
            successBlock(model);
        }
    } failure:^(NSError * _Nonnull error, NSInteger statusCode) {
...
+ (void)getBoardInfoWithConfig:(AgoraBoardInfoConfiguration *)config  success:(OnBoardInfoGetSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {
    
    NSString *url = [NSString stringWithFormat:HTTP_BOARD_INFO, AGORA_EDU_SDK_BASE_URL, config.appId, config.roomUuid];

    NSDictionary *headers = [AgoraHTTPManager headersWithUId:config.userUuid  userToken:config.userToken token:config.token];
    
    [AgoraHTTPManager fetchDispatch:HttpTypeGet url:url parameters:@{} headers:headers parseClass:AgoraBoardModel.class success:^(id _Nonnull model) {
        if(successBlock){
            successBlock(model);
        }
    } failure:^(NSError * _Nonnull error, NSInteger statusCode) {
...
+ (void)getRecordInfoWithConfig:(AgoraRecordInfoConfiguration *)config  success:(OnRecordInfoGetSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {

    NSString *url = [NSString stringWithFormat:HTTP_RECORD_INFO, AGORA_EDU_SDK_BASE_URL, config.appId, config.roomUuid];

    NSDictionary *headers = [AgoraHTTPManager headersWithUId:config.userUuid userToken:nil token:config.token];
    
    [AgoraHTTPManager fetchDispatch:HttpTypeGet url:url parameters:@{} headers:headers parseClass:AgoraRecordModel.class success:^(id _Nonnull model) {
        if(successBlock){
            successBlock(model);
        }
    } failure:^(NSError * _Nonnull error, NSInteger statusCode) {
...
+ (void)assignBreakOutGroupWithConfig:(AgoraAssignGroupInfoConfiguration *)config  success:(OnAssignBreakOutGroupSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {
    ...
    [AgoraHTTPManager fetchDispatch:HttpTypePost url:url parameters:parameters headers:headers parseClass:AgoraAssignGroupModel.class success:^(id _Nonnull model) {
        if(successBlock){
            successBlock(model);
        }
    } failure:^(NSError * _Nonnull error, NSInteger statusCode) {
...
+ (void)fetchDispatch:(HttpType)type url:(NSString *)url parameters:parameters headers:headers parseClass:(Class)classType success:(OnHttpSuccessBlock)successBlock failure:(OnHttpFailureBlock)failureBlock {
    ...
    [AgoraHTTPManager httpStartLogWithType:type url:encodeUrl headers:headers params:parameters];
    
    void (^sucBlock)(id) = ^(id responseObj) {
        [AgoraHTTPManager httpSuccessLogWithType:type url:encodeUrl responseObject:responseObj];
    ...
    void (^failBlock)(NSError *, NSInteger) = ^(NSError *error, NSInteger statusCode) {
        [AgoraHTTPManager httpErrorLogWithType:type url:encodeUrl error:error];
...
+ (void)httpStartLogWithType:(HttpType)type url:(NSString *)url
    ...
    [AgoraEduManager.shareManager logMessage:msg level:AgoraRTELogLevelInfo];
}
