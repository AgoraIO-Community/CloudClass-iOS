...
static AgoraLogOSSManager *manager = nil;

@interface AgoraLogOSSManager()
...
@implementation AgoraLogOSSManager
...
+ (void)initOSSAuthClientWithAccess:(NSString*)access secret:(NSString *)secret token:(NSString *)token endpoint:(NSString*)endpoint {
    ...
    [AgoraLogOSSManager shareManager].ossClient = [[OSSClient alloc] initWithEndpoint:endpoint credentialProvider:credentialProvider clientConfiguration:cfg];
}
...
+ (void)uploadOSSWithAppId:(NSString*)appId access:(NSString*)access secret:(NSString *)secret token:(NSString *)token bucketName:(NSString *)bucketName objectKey:(NSString *)objectKey callbackBody:(NSString *)callbackBody callbackBodyType:(NSString *)callbackBodyType endpoint:(NSString*)endpoint fileURL:(NSURL *)fileURL progress:(void (^ _Nullable) (float progress))progressBlock success:(void (^ _Nullable) (NSString *uploadSerialNumber))successBlock fail:(void (^ _Nullable) (NSError *error))failBlock {
    ...
    if(!AgoraLogOSSManager.shareManager.initOSSAuthClient){
        AgoraLogOSSManager.shareManager.initOSSAuthClient = YES;
        [AgoraLogOSSManager initOSSAuthClientWithAccess:access secret:secret token:token endpoint:endpoint];
    }
    ...
    OSSTask *putTask = [[AgoraLogOSSManager shareManager].ossClient putObject:request];
    [putTask continueWithBlock:^id _Nullable(OSSTask * _Nonnull task) {
        ...
        if (!task.error) {
            ...
            NSError *error = nil;
            AgoraLogOSSModel *model = [AgoraLogOSSModel initWithJsonString:uploadResult.serverReturnJsonString error:&error];
            if(error != nil) {
                if(failBlock != nil) {
                    failBlock(error);
                }
            ...
            if(model.code == 0){
                ...
                if(failBlock != nil) {
                    NSError *error = AgoraLogLocalError(model.code, model.msg);
                    failBlock(error);
                }
            }
