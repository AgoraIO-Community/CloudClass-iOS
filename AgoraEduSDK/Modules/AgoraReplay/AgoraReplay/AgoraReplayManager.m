...
#import "AgoraReplayBoardService.h"
#import "AgoraReplayVideoService.h"

@interface AgoraReplayManager ()<AgoraReplayVideoProtocol, AgoraReplayBoardProtocol> {
    CADisplayLink *_displayLink;
    NSInteger _frameInterval;
    NSTimeInterval _displayDurationTime;
}

@property (nonatomic, assign, readwrite) AgoraReplayPauseReason pauseReason;

@property (nonatomic, strong) AgoraReplayBoardService *boardReplayService;
@property (nonatomic, strong) AgoraReplayVideoService *videoReplayService;
...
@implementation AgoraReplayManager
...
- (instancetype)init {
    if(self = [super init]){
        ...
        _pauseReason = AgoraReplayPauseReasonInit;
...
- (void)joinReplayWithConfiguration:(AgoraReplayConfiguration *)config success:(void (^) (UIView *boardView, AVPlayer *avPlayer))successBlock failure:(void (^) (NSError * error))failureBlock {
    ...
    self.boardReplayService = [AgoraReplayBoardService new];
    self.videoReplayService = [AgoraReplayVideoService new];
...
- (void)videoReplayStartBuffering {
    ...
    //加上 native 缓冲标识
    self.pauseReason = self.pauseReason | AgoraReplayPauseReasonWaitingVideoReplayerBuffering;
...
- (void)videoReplayEndBuffering {
    BOOL isBuffering = !(self.pauseReason & AgoraReplayPauseReasonWaitingWhitePlayerBuffering) || (self.pauseReason & AgoraReplayPauseReasonWaitingVideoReplayerBuffering);
     ...
     */
    if (self.pauseReason & AgoraReplayPauseReasonWaitingWhitePlayerBuffering) {
        [self.videoReplayService pause];
    } else if (!isBuffering && [self.delegate respondsToSelector:@selector(replayEndBuffering)]) {
        [self.delegate replayEndBuffering];
    } else if (self.pauseReason == AgoraReplayPauseReasonNone && [self.delegate respondsToSelector:@selector(replayEndBuffering)]) {
        [self.delegate replayEndBuffering];
    }
     ...
     */
    if (self.pauseReason == AgoraReplayPauseReasonNone) {
        [self.videoReplayService play];
        [self.boardReplayService play];
        _displayLink.paused = NO;
...
#pragma mark AgoraReplayBoardProtocol
- (void)whiteReplayerStartBuffering {
    if ([self.delegate respondsToSelector:@selector(replayStartBuffering)]) {
        [self.delegate replayStartBuffering];
    }
    
    self.pauseReason = self.pauseReason | AgoraReplayPauseReasonWaitingWhitePlayerBuffering;
...
- (void)whiteReplayerEndBuffering {
     ...
     */
    if (self.pauseReason & AgoraReplayPauseReasonWaitingVideoReplayerBuffering) {
        [self.boardReplayService pause];
    } else if (!isBuffering && [self.delegate respondsToSelector:@selector(replayEndBuffering)]) {
        [self.delegate replayEndBuffering];
    } else if (self.pauseReason == AgoraReplayPauseReasonNone && [self.delegate respondsToSelector:@selector(replayEndBuffering)]) {
